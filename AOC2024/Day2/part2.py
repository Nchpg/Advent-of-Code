...
def manager(lines):
    ...
    for line in lines:
        ...
        for i in range(len(T) - 1):
            if good == 0:
                break
            if T[i] < T[i + 1] and ord in [-1, 0]:
                if abs(T[i] - T[i + 1]) > 3:
                    good = 0
                ord = 0
            elif T[i] > T[i + 1] and ord in [-1, 1]:
                if abs(T[i] - T[i + 1]) > 3:
                    good = 0
                ord = 1
            else:
        ...
        if good == 0:
            for j in range(len(T)):
                T1 = [T[i] for i in range(len(T)) if i != j]
                ord = -1
                good = 1
                for i in range(len(T1) - 1):
                    if good == 0:
                        break
                    if T1[i] < T1[i + 1] and ord in [-1, 0]:
                        if abs(T1[i] - T1[i + 1]) > 3:
                            good = 0
                            continue
                        ord = 0
                    elif T1[i] > T1[i + 1] and ord in [-1, 1]:
                        if abs(T1[i] - T1[i + 1]) > 3:
                            good = 0
                            continue
                        ord = 1
