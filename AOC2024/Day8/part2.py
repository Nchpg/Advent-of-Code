...
def manager(lines):
    ...
    for i, j, v in penpgrid(g):
        if v != '.':
            pts[str(v)].append((i, j))

    v = set()
    for i, j in enpgrid(g):
        if g[i][j] == '.':  continue
        for a, b in pts[g[i][j]]:
            if a == i and b == j: continue
            if g[a][b] == g[i][j] and g[a][b] != '.' and (i, j, a, b) not in v:
                dx = i-a
                dy = j-b
                c = 0
                while cnpgrid(g, i+c*dx, j+c*dy):
                    ant.add((i+c*dx, j+c*dy))
                    c += 1
                c = 0
                while cnpgrid(g, a-c*dx, b-c*dy):
                    ant.add((a-c*dx, b-c*dy))
                    c += 1
                v.add((a,b, i, j))
