...
def manager(lines):
    ...
    for i in range(len(L)):
        for j in range(len(L[i])):
            if check(L, i, j, 'A'):
                if check(L, i-1, j+1, 'S') and check(L, i+1, j+1, 'S') and check(L, i+1, j-1, 'M') and check(L, i-1, j-1, 'M'):
                    t += 1
                if check(L, i - 1, j + 1, 'M') and check(L, i + 1, j + 1, 'S') and check(L, i + 1, j - 1,'S') and check(L, i - 1, j - 1,'M'):
                    t += 1
                if check(L, i - 1, j + 1, 'M') and check(L, i + 1, j + 1, 'M') and check(L, i + 1, j - 1,'S') and check(L, i - 1, j - 1,'S'):
                    t += 1
                if check(L, i - 1, j + 1, 'S') and check(L, i + 1, j + 1, 'M') and check(L, i + 1, j - 1,'M') and check(L, i - 1, j - 1,'S'):
                    t += 1
    return t
